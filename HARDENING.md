...
- [x] **Written install procedure** — `docs/WORKSHOP_MACHINE_SETUP.md` (2026-07-28). Covers dependencies, libass verification, model pre-seeding, the rotation budget and an end-to-end verification pass. ⚠ **Not yet executed on a clean machine** — it was recovered from the dev environment by inspection, so the first run on machine A is its real test.
- [ ] ⚠ **Licence check before imaging: `ComfyUI-Olm-DragCrop` may not be redistributed** (`SURVEY.md §3`). A cloned machine image containing it is redistribution. Either obtain the author's written permission, or have students install it themselves from the official repo as part of the workshop, or replace it. Decide before machine A is imaged, not after.
...
- [x] **Ship the subtitle font inside the pack** — done 2026-07-28. Share Tech Mono v1.003 + `OFL.txt` vendored into `assets/fonts/`; `render.encode` defaults `fontsdir` there and warns if the directory is missing. Licence verified from the font's own name table (SIL OFL 1.1) and recorded in `SURVEY.md §3`. The dependency on hand-cloned `ComfyUI_essentials` is gone.
