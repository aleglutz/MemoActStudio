...
ends with a discussion slide, and those slides carry real questions, not
rhetorical ones: each has at least two defensible answers.

| Time | Block | Slides | Min |
|---|---|---|---|
| 0:00 | Title, where we are, schedule | S1–S3 | 5 |
| 0:05 | **Screening: the reel** | S4 | 8 |
| 0:13 | **Visual identity and moodboard — designer** (15 + 7 discussion) | S5 | 22 |
| 0:35 | The film itself, **+ discussion** (5 + 5) | S6–S7 | 10 |
| 0:45 | The ladder: four levels | S8 | 5 |
| 0:50 | Level 1 — quotes, **+ discussion** | S9–S10 | 15 |
| 1:05 | Break | S11 | 5 |
| 1:10 | Level 2 — sound, **+ discussion** | S12–S15 | 15 |
| 1:25 | Level 3 — sharpening, **+ discussion** | S16–S18 | 15 |
...
> **Numbering note.** The film now comes before the ladder — you look at the
> material first and only then at what can be done to it. S22 is not its own row
> in the run sheet: it shares the closing five minutes with S23, three and two.
...
> **Say:** You are looking at steps 5 and 6 of the workflow. We are at step 3.
...
## S5 Visual identity and the moodboard — designer

Fifteen minutes on the idea behind the project's visual identity and how it
connects to the students' own films, then seven minutes of discussion. Slides
supplied separately.
...
3. **It already carries the museum's own burned-in captions** elsewhere in the
   reel. Ours would argue with theirs. This stretch is clean, which is part of
   why it was chosen.

> **The black-and-white is already a decision.** The scan carries a constant
> warm cast — a few levels more red than green, everywhere, all the way
> through. Nobody chose that in the room in 1945. Somebody chose it at a scanner.

## S7 The film — discussion

**Three questions before anyone touches a tool.**

1. **You are never intervening in raw material. You are intervening in someone
   else's intervention.** The crop, the transfer, the tint, the museum's own
   captions — all of it arrived as decisions. Does that make your changes more
   defensible, or less?
2. **This stretch was chosen partly because the museum's captions are absent
   from it.** Is choosing the convenient thirty seconds already an edit? What
   would you have to disclose?
3. **Nothing in this film is signed.** No cameraman, no editor, no scanning
   technician. If you cannot credit the people whose decisions you inherited,
   what can you honestly say about provenance?

> **Say:** Hold on to the tint. In forty minutes a model will be asked to
> colourise this film — and the thing it is "restoring" colour to is a frame
...
## S8 The ladder — four levels of intervention
...
## S9 Level 1 — the rule, and what the tool cannot do
...
stock graph, which is visibly poorer in two ways:
...
> **Say:** The second limit never shows on *this* film. We measured the strip
> where the caption sits, every half second across the whole cut: 14 to 65 out
> of 255. Dark everywhere. So the captions look fine, the defect is real, and
> your test footage is hiding it. **A flaw your material happens to conceal is
> the expensive kind** — it surfaces on the next film, in front of an audience.
...
2. **This reel already carries the museum's own burned-in captions.** If yours
   contradicts theirs on the same frame, whose survives — and who decides?
...
## S11 Break — five minutes

Back at the hour. After the break: the film has no sound at all, and we make
some.

> **Say:** Before you go — the next three levels each cost real money and real
> time, and level 1 cost neither. Keep that in mind when you decide how far up
> the ladder your own project actually needs to go.

*(5 min.)*

---

## S12 Level 2 — the film is silent, so everything you hear is invented
...
> museum practice. **Nobody captions a room tone.** A generated hall murmur is
> received as atmosphere rather than as content, and it passes unchallenged in
> places where an invented image would be caught immediately. That asymmetry is
> worth more discussion than the technique.

## S13 Level 2 — how it is made, in eight nodes

Everything is already on Cloud: no downloads, no Hugging Face account, no
licence to accept. This is the graph in the handout — read it here, build it at
home.
...
> encoders take CLIP from **node 2**, not from the checkpoint. The checkpoint
> has the model and the VAE but **no text encoder** — wire CLIP from node 1 and
> you get `CLIP = None` and an error that blames the checkpoint file.

## S14 Level 2 — four layers, and why two were rewritten
...
Two of those are **second attempts**, and the rewriting is the real skill here.

- The first **bed** came back flat broadband hiss — a bad recording rather than
  a room. The rewrite named what was *in* the room: *many men murmuring quietly
  at a distance… occasional cough and chair creak.*
...
> **Say:** Six seconds of GPU a layer whether you ask for twelve seconds of
> audio or thirty-two — four layers is about three cents. When a layer is wrong,
> do not roll the seed. Ask what the model misheard, and say the missing thing.
> Both fixes here were words, not numbers.
...
1. **Nobody captions a room tone. Should they?** What would an honest sound
   credit look like on a thirty-second video — and would anyone read it?
2. **The narration in the reel you saw is a real recorded human voice.** If it
   ...
   crowd murmur? If it does, what is the difference you are pointing at?
...
*(15 min for S12–S15. Cut the eight-node table first if the room is slow — it is
...
## S18 Level 3 — twenty pixels, and discussion

**These faces are about twenty pixels tall in the original.** No model can
recover a face that small. It can only make one up.
...
## S21 Level 4 — the flicker, and discussion
...
2. **Our fix keeps the film's own luminance, so no shape can move.** Does that
   make the colour honest — or only make the invention harder to see? The
   colours on frame 660 are still wrong; they are just wrong with crisp edges.
