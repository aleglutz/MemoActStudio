...
| **pedalboard** (Spotify) 0.9.19 — a *declared dependency of this pack*, not a neighbour | **GPL-3.0** | **verified 2026-09-03** from the installed wheel's own metadata. Adopted for `nodes_voice.py`: Rubber Band time-stretch, and the JUCE compressor and gate. GPLv3 is the strongest obligation in this ledger because we **import** it rather than sit beside it in a graph — and it is satisfied by construction: this pack is GPL-3.0-or-later already, for the same reason (ComfyUI's API). The bundled Rubber Band library carries the same licence. Nothing here may be re-licensed more permissively while this import stands |
