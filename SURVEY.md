...
| **ComfyUI-Olm-DragCrop** | **source-available, NOT open source** | ⚠ **installed 2026-08-10 — read this before relying on it.** The author's own words: "source-available, but not open-source under a standard open-source license, and not freeware." Using it is unrestricted and *"any results you create with it are yours"*, so the museum reel is unaffected. But **"redistribution … is strictly prohibited without explicit written permission"**, which collides with two planned uses: bundling it inside `comfyui-memoacts`, and shipping it in the cloned machine image for the September workshop (`HARDENING.md`). Neither is permitted as things stand. Three ways out, in order of cost: ask the author (he explicitly invites gray-area questions and this is a museum/education case); have each student install it themselves from the official repo, which is plain *use*; or write our own crop node, which is the fallback already named in `HANDOFF_comfy_threeband.md` §7. |
