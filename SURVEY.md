...
| 5b | Subtitle burn-in, multilingual passes (§5.5) | **ComfyUI-Whisper** (yuvraj108c) & forks draw text per-frame via PIL — no `.ass` styling, no safe zones, no sidecars, no N-language passes over one visual render | **Build** | `.ass` + ffmpeg libass per spec. Existing nodes solve a different problem (ASR captioning of generated clips). |
...
**Net build surface** (the pack earns its existence here): motion engine + resolution guard, shot table + alignment, layer-stack compositor, `.ass` multilingual subtitles, thin audio/encode nodes. Everything routine is adopted.
